...
def test_cryptokeys_issue(session_auth):
    ...
    r = session_auth.post('/api/cryptokeys/issue/', json={
        'frontend_id': frontend.id,
    })
    assert r.status_code == 200, f'Invalid status {r.status_code}'

def test_cryptokeys_sshkey(session_auth, mocker):
    mocker.patch(
    ...
    r = session_auth.post('/api/cryptokeys/sshkey/', json={
        'type': 'ssh-rsa',
        'public': SSH_KEY,
    })
    assert r.status_code == 200, f'Invalid status {r.status_code}'
