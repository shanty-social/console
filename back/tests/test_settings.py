...
def test_create_settings_text(session_auth):
    "Ensure we can create settings using text format."
    r = session_auth.post('/api/settings/?format=text', data='FOO=foo')
    assert r.status_code == 201, f'Invalid status {r.status_code}'
    r = session_auth.post('/api/settings/?format=text', data='bar=BAR')
    assert r.status_code == 201, f'Invalid status {r.status_code}'
